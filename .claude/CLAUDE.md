...
> API/화면 구현을 설계 → 구현 단계로 정리한 상세 문서: [docs/api-design.md](../docs/api-design.md), [docs/ui-design.md](../docs/ui-design.md)
...
- [x] GET /api/showtimes/:id/seats — 좌석 + isBooked, 상영 가격(price), 로그인 시 내가 이미 예매한 좌석 수(myReservedCount) 포함. 비로그인도 접근 가능

### 3. reservations — 흐름: 영화 → 영화관/상영시간 → 좌석

- [x] POST /api/reservations — seatIds 1~6개(최대 6개, 요청 단위), 트랜잭션 + `(showtimeId, seatId)` unique 제약으로 중복예매 차단, 이미 예약 시 409
- [x] 상영별 사용자 누적 좌석 상한 — 같은 상영에 대해 사용자가 이미 예매(CONFIRMED)한 좌석 수 + 새 요청 좌석 수가 6석을 넘으면 400(SEAT_LIMIT_EXCEEDED)
